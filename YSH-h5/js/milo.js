...
(function(){	
	var config = {
		loaderPath : location.protocol+"//ossweb-img.qq.com/images/js/mobile/",
		version : "20150510",
		expires : 30000
	}
...
extend( string, {
	 ...
	 */
	isURL : function(str){
		return /^http|https:\/\/[A-Za-z0-9-_]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/.test(str);
	},
	/**
	 * 是否数字字符串
	 * @param {string} str 字符串
	...
	unSerialize : function(jsonStr, de){
		...
		for (var i = 0; i < obj1Ret.length; i++) {
			...
			if (ret2.length >= 2) {
				...
				//修复#hash值时的bug (add by dickma  2015.05.11 MSDK参数解析)
				if (ret1.indexOf("#")>-1){
					ret1=ret1.split("#")[0];
				}
...
(function(){
	...
	function getAmsFile(amsActivityId,flowId, callback){
		...
		/*========AMS活动配置文件路径优化改造 Start==========*/
		var sActId=amsActivityId+"";
		var iActId=Number(amsActivityId);
		var _url="";
		if(iActId>=125300){//走新路径
	...
	function getDesc(obj, callback){
		...
		/*========AMS活动配置文件路径优化改造 Start==========*/
		var sActId=obj.actId+"";
		var iActId=Number(obj.actId);
		var _url="";
		if(iActId>=125300){//走新路径
			_url = location.protocol+"//" + window.location.host + "/comm-htdocs/js/ams/actDesc/" + sActId.substr(sActId.length-3) + "/" + sActId + "/act.desc.js";
		}else{//走老路径
...
/*******判断出有传参数mconsole=1则启动mconsole********/
if(milo.request("mconsole")==1){
   need('util.zepto', function($){
	 var mcon_url = location.protocol+"//ossweb-img.qq.com/images/js/mobile/biz/widget/mtest/mconsole.js";
	 loadScript(mcon_url);
   });
}
...
//add susahuang 2017/5/2 清除lg_srouce,ams_game_appid
...
/*  |xGv00|6247a2a76808e5d68de54cfb7e075f08 */